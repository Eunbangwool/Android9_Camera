...
주소·포트·접속 ID/PW 는 cast 앱 화면 좌상단에 표시된다(`📱 http://…:<포트>` / `🔒 ID: admin PW: …`).
`/ch0`, `/ch1` 로 개별 채널도 접근 가능. (MJPEG over HTTP — 폰에 앱 설치 불필요.)

- **인증**: HTTP Basic 인증(ID `admin` + 기기에서 무작위 생성된 비밀번호). 비밀번호는
  SharedPreferences 에 저장되고 화면에 표시된다(공개 repo 소스엔 없음).
- **포트**: 8090부터 비어있는 포트를 자동 선택(8080 은 기기 타서비스 점유). 실제 포트는 화면 표시.

#### 외부(인터넷)에서 접속

태블릿은 공유기 안쪽 사설 IP 라 밖에서 바로 못 닿는다. 두 가지 방법:
- **포트포워딩**: 공유기에서 외부포트 → `<태블릿 IP>:<표시된 포트>` 포워딩(+공인 IP/DDNS).
  인터넷 노출이므로 위 Basic 인증 필수(이미 적용됨). 접속: `http://<공인IP 또는 DDNS>:<외부포트>`
- **Tailscale 등 VPN 메시**: 공인 IP 없이(CGNAT) 가능. 태블릿·폰에 설치 후 태블릿 Tailscale IP:포트로 접속.
